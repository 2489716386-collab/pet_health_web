  ...
  <div class="app-container">
    <el-card>
      <el-form :inline="true" :model="queryParams" class="search-form">
        ...
        <el-form-item label="目标内容ID">
          <el-input v-model="queryParams.targetId" placeholder="搜索目标ID" clearable style="width: 120px" />
        </el-form-item>
        ...
        <el-form-item label="举报时间">
          <el-date-picker v-model="timeRange" type="daterange" range-separator="至" value-format="YYYY-MM-DD" clearable @change="handleTimeChange" />
        </el-form-item>
      ...
      <el-table :data="tableData" border stripe v-loading="loading">
        ...
        <el-table-column prop="status" label="处理状态" align="center" width="120">
          <template #default="scope">
            <el-tag v-if="scope.row.status === 0 || scope.row.status === 'PENDING'" type="warning">待处理</el-tag>
            <el-tag v-else-if="scope.row.status === 1 || scope.row.status === 'IGNORED'" type="info">已忽略</el-tag>
            <el-tag v-else-if="scope.row.status === 2 || scope.row.status === 'DELETED'" type="danger">已删除内容</el-tag>
          </template>
        </el-table-column>
      ...
      <div class="pagination-container">
        <el-pagination v-model:current-page="queryParams.pageNum" v-model:page-size="queryParams.pageSize" :page-sizes="[10, 20, 30, 50]" layout="total, sizes, prev, pager, next, jumper" :total="total" @size-change="handleSizeChange" @current-change="handleCurrentChange" />
      </div>
    </el-card>
  </div>
</template>
...
const queryParams = reactive({
  pageNum: 1, pageSize: 20, targetType: '', status: null, startDate: '', endDate: '', reason: '', targetId: ''
})
...
const resetQuery = () => {
  Object.assign(queryParams, { targetType: '', status: null, startDate: '', endDate: '', reason: '', targetId: '' })
...
onMounted(() => {
  if (route.query.targetId) queryParams.targetId = route.query.targetId
  if (route.query.targetType) queryParams.targetType = route.query.targetType // 取消 Number 转换
  fetchList()
})
</script>
